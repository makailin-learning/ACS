...
class ACS(nn.Module):
    def __init__(self, in_channels, kernel_size=3, stride=1, padding=1, deploy=False, activation=None):
        ...
        if deploy:
            ...
            self.acs_3x3.add_module("conv1",nn.Conv2d(in_channels=in_channels, out_channels=mid_channels,
            ...
            # 3x3分支
            self.acs_main = nn.Conv2d(in_channels=in_channels, out_channels=mid_channels,
                                      kernel_size=kernel_size, stride=stride, padding=padding, bias=False)
            self.acs_main_bn = nn.BatchNorm2d(mid_channels)

            # 1x1-bn-avg分支
            self.acs_avg = nn.Sequential()
            self.acs_avg.add_module("conv1",nn.Conv2d(in_channels=in_channels, out_channels=mid_channels,
                                                      kernel_size=1, stride=1, padding=0, bias=False))
        ...
        self.acs_1x1.add_module("conv",nn.Conv2d(in_channels=3*mid_channels,out_channels=in_channels,kernel_size=1,
